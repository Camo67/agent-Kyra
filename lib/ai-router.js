...
const REASONING_MODEL = "huihui_ai/deepseek-r1-abliterated";

const DETECTION_ORDER = ["ollama", "lmstudio", "lightrag", "agent"];
...
function withTimeout(options = {}, timeoutMs = 30000) {
  return {
    ...options,
...
async function pingLightRAG() {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 5000);

  try {
    const lightragURL = process.env.LIGHTRAG_URL || "http://localhost:8000";
    const response = await fetch(`${lightragURL}/v1/models`, {
      method: "GET",
      signal: controller.signal
    });
...
async function detectBackend() {
  for (const backend of DETECTION_ORDER) {
    ...
    if (backend === "lightrag" && await pingLightRAG()) {
      return { backend, baseURL: process.env.LIGHTRAG_URL || "http://localhost:8000" };
    }
...
async function sendToOllama(payload, options = {}) {
  const resolvedOptions = withTimeout(options, 60000);
...
async function sendToLightRAG(payload, options = {}) {
  const resolvedOptions = withTimeout(options, 60000);
  const lightragURL = process.env.LIGHTRAG_URL || "http://localhost:8000";

  const messages = payload.messages || [
    { role: "system", content: payload.context || DEFAULT_SYSTEM_PROMPT },
    { role: "user", content: payload.prompt || payload.userText }
  ];

  const response = await fetch(`${lightragURL}/v1/chat/completions`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      model: payload.model || "lightrag",
      messages: messages,
      stream: false,
      temperature: payload.temperature ?? 0.7,
    }),
  ...
  if (!response.ok) {
    const errorBody = await readErrorBody(response);
    throw new Error(`LightRAG Error: HTTP ${response.status} ${response.statusText}: ${errorBody}`);
  }
...
async function sendCompletion(payload, options = {}) {
  ...
  } else if (target === "lightrag") {
    return await sendToLightRAG(payload, options);
  } else if (target === "reasoning") {
    // Force use of DeepSeek-R1-Abliterated for complex reasoning tasks
    const reasoningPayload = {
      ...payload,
      model: REASONING_MODEL
    };
    return await sendToOllama(reasoningPayload, options);
  } else {
...
class InferenceBridge {
  ...
  sendToLightRAG(payload, options = {}) {
    return sendToLightRAG(payload, options);
  }
  ...
  // Compatibility methods for old InferenceBridge interface
  detect() {
    return this.detectBackend().then(b => b?.backend || null);
  }

  chat(text) {
    return this.sendCompletion({ userText: text }).then(res => extractAgentText(res));
  }

  reset() {
    if (this.options.buffer && typeof this.options.buffer.clear === "function") {

...
class MIDITokenizer:
    ...
    def tokens_to_event(self, tokens: List[int]) -> dict:
        ...
        return {"channel": ch, "pitch": pitch, "velocity": vel, "duration": dur, "noteName": note}
...
class SparseEventRouter(nn.Module):
    ...
    def forward(self, x: torch.Tensor, channel_hint: Optional[int] = None):
        ...
        self.load_balancing_loss = (
            gate_scores.mean(dim=[0, 1]) * F.log_softmax(gates, dim=-1).mean(dim=[0, 1])
        ).sum()
...
class DurationGatedMemory:
    ...
    def _prune(self):
        ...
        while len(self.slots) > self.max_slots:
            min_idx = min(
                range(len(self.slots)), key=lambda i: self.slots[i]["event"].get("velocity", 64)
            )
...
class MIDIMXitNet(nn.Module):
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.memories = nn.ModuleList(
            [MotifMemory(d, max_motifs=config.max_motifs) for _ in range(config.n_layers)]
        )
    ...
    def forward(
        ...
        if targets is not None:
            ...
            ce_loss = F.cross_entropy(
                logits.view(-1, self.config.vocab_size), targets.view(-1), ignore_index=PAD_ID
            )
...
class Trainer:
    def __init__(self, model: MIDIMXitNet, config: ModelConfig):
        ...
        self.opt = torch.optim.AdamW(
            model.parameters(), lr=config.lr, weight_decay=0.01, betas=(0.9, 0.95)
        )
...
def main():
    ...
    if args.info:
        ...
        print(
            f"  SparseEventRouter × {config.n_layers}: fires only top-{config.top_k} of {CHANNELS} channels"
        )
        print(f"  MotifMemory × {config.n_layers}: {config.max_motifs} learnable patterns")
        print("  DurationGatedMemory: TTL-based runtime slots")
        print("  MXitOutputHead: generates compressed tokens directly")
        return

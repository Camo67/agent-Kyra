...
class Huawei5GRotator:
    def __init__(
        self, huawei_interface_ip: str, router_ip: str, router_user: str, router_pass: str
    ):
        """
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Received status code {response.status_code} on attempt {attempt + 1}"
                    )
...
def main():
    ...
    parser.add_argument(
        "--interface-ip", required=True, help="Huawei interface IP (e.g., 192.168.8.100)"
    )
    parser.add_argument(
        "--router-ip", default="http://192.168.8.1", help="Router IP (default: http://192.168.8.1)"
    )
    ...
    parser.add_argument(
        "--rotate-only", action="store_true", help="Rotate IP only without scraping"
    )
